<template>
  <div v-if="items.length > 0" class="cart">
    <div class="wrapper">
      <h3 class="title">My basket</h3>
      <div class="basket-items">
        <div v-for="item in items" :key="item.id" class="basket">
          <img
            class="basket__img"
            :src="parsedImgPath(item.id)"
            :alt="item.title"
          />
          <div class="basket__info">
            <p class="basket__name">{{ item.title }}</p>
            <div class="basket__controller">
              <button
                class="basket__button basket__button_minus"
              ...
              <button
                class="basket__button basket__button_plus"
          ...
          <button
            class="basket__button basket__button_del"
            @click="deleteBuyItem(item.id)"
          >
            <svg
              width="19"
              height="18"
    ...
    <div class="wrapper">
      ...
      <div class="promo">
        ...
        />
        <button class="promo__button" @click="checkPromo()">Apply</button>
        <div v-if="promoActive" class="promo__desc promo__desc_active">
          <p class="promo__text">Promo code successfully redeemed!</p>
          <p class="promo__text">
            Sale on your order is
            <span class="promo__text_discount">{{ code.discount }}%</span>
          </p>
        </div>
        <div
          class="promo__desc promo__desc_err"
          v-else-if="!promoActive && check"
        >
          <p class="promo__text">
            Sorry, this promocode is invalid! Try another one.
          </p>
        </div>
      </div>
    </div>
    <div class="wrapper">
      <div class="total">
        <div class="total__item">
          ...
          <span class="total__value"> $ {{ subTotal }} </span>
        </div>
        <div class="total__item">
          <span class="total__label"> Tax </span>
          <span class="total__value"> $ {{ tax }} </span>
        </div>
        <div class="total__item">
          <span class="total__label"> Shipping </span>
          <span class="total__value"> $ {{ shipping }} </span>
        </div>
        <div class="total__item">
          <span class="total__label total__label_main"> Total </span>
          <span class="total__value total__value_main"> $ {{ total }} </span>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    subTotal() {
      let fullPrice = this.items.reduce((subtotal, product) => {
      ...
      if (!this.promoActive) {
        ...
        let percent = (fullPrice * this.code.discount) / 100;
        let newPrice = fullPrice - percent;
        return newPrice;
      }
    },
    total() {
      return this.tax + this.shipping + this.subTotal;
    },
  },
  methods: {
    ...mapMutations(["incrementItemQuantity"]),
    ...mapActions(["decrementItemQuantity"]),
    ...mapMutations(["deleteBuyItem"]),
    ...
    checkPromo() {
      this.check = true;
      let code = this.promoCodes.find((item) => item.value === this.promo);
      if (code) {
    ...
    parsedImgPath(id) {
      return require(`@/assets/mockImg/${id}.png`);
    },
  },
};
</script>
